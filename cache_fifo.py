...
def start(file, loop_size, l1, l2, l3):
    ...
    with open("./Result/FIFO/"+str(L1_size)+"config.txt", "w") as f:
        f.write("Location: ./Result/FIFO/"+str(L1_size)+"\nPolicy: FIFO\nData: %s\nL1: %d\nL2: %d\nL3: %d"% ("cc1", L1_size, L2_size, L3_size))
    with open('./Result/FIFO/'+str(L1_size)+'hit.txt','wb') as f:
        pickle.dump(stat.is_hit, f)
    with open('./Result/FIFO/'+str(L1_size)+'time.txt','wb') as f:
